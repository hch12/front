<template>
  ...
  const queryNameSuggestions = async (queryString, cb) => {
    try {
      ...
      const res = await request.get(APIs.NAME_SUGGEST, {
        params: { 
          keyword: queryString ,
          id: user.id
        }
  ...
  const validateNameInput = () => {
    if (displayName.value && !nameMap.value[displayName.value]) {
      ...
      searchParams.nameId = nameMap.value[displayName.value]
    }
  }
...
const fetchData = async () => {
  // 没有搜索条件且是第一页时直接返回
  if (!hasSearchCondition() && pagination.pageNum === 1) {
    tableData.value = []
    pagination.total = 0
    return
  }
